...
app.post('/api/wilayah/kecamatan', authenticateToken, roleGuard(['admin']), async (req: any, res) => { try { const { nama, kota_id } = req.body; if (!nama || !kota_id) throw new Error('Nama kecamatan dan kota ID wajib diisi'); await db.query('INSERT INTO kecamatan (nama, kota_id) VALUES (?, ?)', [nama, kota_id]); res.json({ success: true }); } catch (err: any) { res.status(400).json({ message: err.message }); } });
app.delete('/api/wilayah/kecamatan/:id', authenticateToken, roleGuard(['admin']), async (req: any, res) => { try { await db.query('DELETE FROM kecamatan WHERE id = ?', [req.params.id]); res.json({ success: true }); } catch (err: any) { res.status(500).json({ message: err.message }); } });

app.post('/api/wilayah/kelurahan', authenticateToken, roleGuard(['admin']), async (req: any, res) => { try { const { nama, kecamatan_id } = req.body; if (!nama || !kecamatan_id) throw new Error('Nama kelurahan dan kecamatan ID wajib diisi'); await db.query('INSERT INTO kelurahan (nama, kecamatan_id) VALUES (?, ?)', [nama, kecamatan_id]); res.json({ success: true }); } catch (err: any) { res.status(400).json({ message: err.message }); } });
app.delete('/api/wilayah/kelurahan/:id', authenticateToken, roleGuard(['admin']), async (req: any, res) => { try { await db.query('DELETE FROM kelurahan WHERE id = ?', [req.params.id]); res.json({ success: true }); } catch (err: any) { res.status(500).json({ message: err.message }); } });
