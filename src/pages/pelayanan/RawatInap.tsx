...
export default function RawatInap() {
  ...
  const handleDeleteRecord = async (id: number) => {
    Swal.fire({
      title: 'Hapus Kunjungan Ranap?',
      text: 'Apakah Anda yakin ingin menghapus kunjungan pasien rawat inap ini secara permanen?',
      icon: 'warning',
  ...
  filteredRecords.forEach(r => {
    if (r.icd_masuk) {
      icdCountMap[r.icd_masuk] = (icdCountMap[r.icd_masuk] || 0) + 1;
    }
    if (r.icd_pulang) {
      icdCountMap[r.icd_pulang] = (icdCountMap[r.icd_pulang] || 0) + 1;
    }
  });
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-150/60 shadow-xs space-y-4">
                ...
                {top10Diagnosa.length === 0 ? (
                  <div className="text-center py-10 text-slate-350 text-xs font-mono border border-dashed border-slate-200 rounded-2xl">
                    Belum ada diagnosa (ICD-10) tercatat pada rekam medis kunjungan Rawat Inap
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {top10Diagnosa.map((item, index) => {
                      const percentage = totalVisits > 0 ? Math.round((item.count / (totalVisits * 2)) * 100) : 0;
                      return (
