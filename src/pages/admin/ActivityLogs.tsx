...
export default function ActivityLogs() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div id="logs-filters-container" className="bg-white rounded-2xl border border-slate-150 p-5 shadow-sm space-y-4">
        <div className="flex items-center space-x-2 pb-2 border-b border-slate-100">
          <Filter className="h-4 w-4 text-teal-600" />
          <h2 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Filter &amp; Penelusuran</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {/* Search box */}
          <div>
            <label htmlFor="filter-search-input" className="block text-xxs font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Kata Kunci</label>
          ...
          {/* Action Type */}
          <div>
            <label htmlFor="filter-action-select" className="block text-xxs font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Tipe Aksi</label>
            <select
              id="filter-action-select"
              value={selectedAction}
              onChange={(e) => setSelectedAction(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 text-xs text-slate-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all cursor-pointer font-sans"
              style={{ minHeight: '38px' }}
            >
          ...
          {/* Module */}
          <div>
            <label htmlFor="filter-module-select" className="block text-xxs font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Fitur / Modul</label>
            <select
          ...
          {/* Date range helper */}
          <div>
            <label htmlFor="filter-date-select" className="block text-xxs font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Rentang Waktu</label>
            <select
      ...
      <div id="logs-list-card" className="bg-white rounded-2xl border border-slate-150 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-100 text-left">
              <thead className="bg-slate-50">
                <tr className="text-slate-500 text-xxs font-extrabold uppercase tracking-widest font-display">
                  ...
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-800">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="font-mono text-xs text-slate-600 font-normal">{log.email}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      <span className={`inline-block text-[9px] font-extrabold uppercase px-2.5 py-1 rounded-lg tracking-wider ${getActionBadge(log.action_type)}`}>
                        {log.action_type}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-teal-700 font-normal">
                      <div className="flex items-center space-x-2">
                        <Layers className="h-4 w-4 text-teal-600/75 flex-shrink-0" />
                        <span>{log.module_name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-slate-600 font-normal font-sans leading-relaxed break-words max-w-md">
                      {log.description}
                    </td>
                  </tr>
                ))}
        ...
        {/* Footer info banner */}
        {!loading && !error && (
          <div className="bg-slate-50 px-6 py-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between text-xs text-slate-500 gap-2 font-semibold">
            <div className="flex items-center space-x-2">
              <CheckCircle2 className="h-4 w-4 text-teal-600" />
              <span>Sistem pencatatan log aktivitas aktif secara real-time.</span>
            </div>
            <div className="font-mono">
              Total Log: <span className="text-teal-600 font-extrabold">{filteredLogs.length}</span> baris
            </div>
          </div>
        )}
      </div>
